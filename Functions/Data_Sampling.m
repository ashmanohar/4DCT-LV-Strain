...
for j = info.timeframes
    ...
    z_lim(1) = z_lim(1) - info.apical_basal_threshold(2)*(z_lim(1) - z_lim(2)); %Basal
    z_lim(2) = z_lim(2) + info.apical_basal_threshold(1)*(z_lim(1) - z_lim(2)); %Apical
        ...
        while 2*pi - dummy > bin_width %Azimuthal loop
            ...
            if nnz(list) == 0
                ...
                if info.endo_strains
                    data{k}(4,c) = NaN; %4th row is Ecc
                    data{k}(5,c) = NaN; %5th row is Ell
                end    
            else
                ...
                data{k}(3,c) = locb;
                if info.endo_strains
                    data{k}(4,c) = Mesh(j).Ecc(locb);
                    data{k}(5,c) = Mesh(j).Ell(locb);
                end
            end
