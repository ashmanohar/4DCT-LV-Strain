function [Mesh, info] = Mesh_Extraction(info)
...
for j = 1:length(info.timeframes) % Time frame loop
    ...
    if info.fill_paps
        ...
        temp2 = imdilate(temp2,strel('sphere',7));
    ...
    if info.timeframes(j) == info.template
        ...
        tol = 10; scaling_factor = 6;
        % Determining limits of LV for croppoing
        [row,col,fr] = ind2sub(size(I),find(I==1));

        % Ensuring cropping doesn't exceed bounds of image; cropping done to speed up process
        if min(row)-tol <= 0; crop(1) = 1; else; crop(1) = min(row)-tol; end
        if min(col)-scaling_factor*tol <= 0; crop(3) = 1; else; crop(3) = min(col)-scaling_factor*tol; end
        if min(fr)-tol <= 0; crop(5) = 1; else; crop(5) = min(fr)-tol; end

        if max(row)+tol > size(Irot,1); crop(2) = size(Irot,1); else; crop(2) = max(row)+tol; end
        if max(col)+tol > size(Irot,2); crop(4) = size(Irot,2); else; crop(4) = max(col)+tol; end
        if max(fr)+tol > size(Irot,3); crop(6) = size(Irot,3); else; crop(6) = max(fr)+tol; end

        Irot = Irot(crop(1):crop(2),crop(3):crop(4),crop(5):crop(6));

        % Calculating angles - z
        figure('pos',[10 10 1000 1000])
        imagesc(Irot(:,:,round(size(Irot,3)/2))); hold on
        ...
        figure('pos',[10 10 1000 1000])
        imagesc(squeeze(Irot(:,round((size(Irot,2)+(scaling_factor-1)*tol)/2),:)));
        axis equal; colormap gray; caxis([-100 700])
        ...
        figure('pos',[10 10 1000 1000])
        imagesc(squeeze(Irot(:,:,round(size(Irot,3)/2))));
        axis equal; colormap gray; caxis([-100 700])
        ...
        clear Irot yp zp t_z tform_z t_x tform_x temp;
    ...
    %Extracting the vertices corresponding to faces of largest component
    %which is LV
    temp = temp(:);
    temp = unique(temp);
