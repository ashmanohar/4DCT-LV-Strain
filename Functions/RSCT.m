...
%Identifying faces belonging to valve planes
nan_ind = isnan(ED_Ar);
...
for j = 1:length(info.timeframes)
    ...
    % NaN-ing values of faces belonging to valve planes 
    Avg_Ar(nan_ind) = NaN;
    ...
    for j4 = 1:length(template)
        ...
        Mesh(info.timeframes(j)).RSct_vertex(j4) = mean(Mesh(info.timeframes(j)).RSct(row),'omitnan');
